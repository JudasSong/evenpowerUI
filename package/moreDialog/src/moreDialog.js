...
const EpMoreDialog = `<div class="epDialogWrapper" v-if="show">
    <el-dialog 
...
export default (options) => {
  ...
  var randomNum = Math.floor(Math.random() * (1 - 100) + 100);
  if (options.parentEl) {
    options.parentEl.appendChild(element);
  } else {
    document.getElementsByTagName('body')[0].appendChild(element);
  }
  element.id = `epMoreDialog${randomNum}`;

  epVue = new Vue({
    el: `#${element.id}`,
    template: EpMoreDialog,
    // templateUrl: "./moreDialog.vue",
    data() {
      return {
        title: options.title,
        style: options.style,
