    ...
    <div v-for="title in (Object.keys(data))" class="group-container">
      ...
      <div class="menu-items" v-for="nav in data[title]" v-if="nav.desc">
        ...
        <div class="menu-items-son" v-for="item in nav.items">
          <router-link :to="{name: item.name}" :class="$route.name===item.name ? 'active' : ''" class="menu-component">{{item.desc}}
          </router-link>
        </div>
      </div>
    </div>
...
<script>
import routerConfig from '@/router/router.json/'
...
.asideMenu {
  ...
  .menu-items {
    ...
    .menu-group {
      padding: 4px 0;
      font-size: 15px;
      color: lighten(@color-primary, 24%);
      font-weight: 600;
    }
